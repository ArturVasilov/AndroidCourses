...
/**
 * The {@link org.andengine.util.adt.queue.concurrent.PriorityBlockingAggregatorQueue} is a thread-safe queue that internally holds multiple queues each having their own priority.
 * {@link #peek()}, {@link #poll()} and {@link #take()} always return the head of the highest priority internal queue.
 * The {@link org.andengine.util.adt.queue.concurrent.PriorityBlockingAggregatorQueue} is i.e. useful in networking situations where different {@link Thread}s (producers) put different messages of different priority and the network {@link Thread} (consumer) should always send the highest priority message first.
 *
