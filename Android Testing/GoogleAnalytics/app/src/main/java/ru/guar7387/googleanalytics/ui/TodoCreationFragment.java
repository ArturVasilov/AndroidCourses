...
public class TodoCreationFragment extends Fragment implements View.OnClickListener {
    ...
    private static final String TAG = TodoCreationFragment.class.getSimpleName();
        ...
        switch (v.getId()) {
            ...
            case R.id.fab:
                if (mTodoText.getText() == null || mTodoText.getText().toString().isEmpty()) {
                    ...
                    application.sendEventReport(TAG, "On fab click", "User didn't input todo task name");
                    return;
                }
                else {
                ...
                if (mDateEdit.getText() == null || mDateEdit.getText().toString().isEmpty()) {
                    ...
                    application.sendEventReport(TAG, "On fab click", "User didn't select date");
                    return;
                }
                else {
                ...
                if (mTimeEdit.getText() == null || mTimeEdit.getText().toString().isEmpty()) {
                    ...
                    application.sendEventReport(TAG, "On fab click", "User didn't select tmie");
                    return;
                }
                else {
                    mTimeEdit.setBackgroundColor(Color.TRANSPARENT);
                }
                TodoItem item = new TodoItem(mTodoText.getText().toString(), mCalendar.getTimeInMillis());
                application.sendEventReport(TAG, "On fab click", "New task was created - " + item.getTask());
                ((OnTodoCreated) getActivity()).onTodoItemCreated(item);
                break;
        }
    }
